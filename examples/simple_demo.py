...
data = {
    ...
    "Food_Group": ["Cereals", "Cereals", "Pulses", "GLV", "OV", "Fruits", "Oils"],
...
for nutrient, target in EAR.items():
    expr = sum(
        df.loc[i, nutrient] * food_vars[df.loc[i, "Food_Item"]] for i in range(len(df))
    )
    g = Goal(
        name=nutrient,
        expression=expr,
...
# Cost expression
cost_expr = sum(
    df.loc[i, "Cost_NT"] * food_vars[df.loc[i, "Food_Item"]] for i in range(len(df))
)
...
for nutrient in EAR.keys():
    expr = sum(
        df.loc[i, nutrient] * result["variables"][df.loc[i, "Food_Item"]]
