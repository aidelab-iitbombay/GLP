...
Linear programming provides mature and efficient tools for expressing feasibility constraints and optimizing a single scalar objective. When analysts face problems involving multiple objectives, common practice is to rely on multiobjective linear programming techniques such as weighted sum scalarization, ε-constraint formulations, Pareto-based analyses, and interval-based multiobjective linear programming. In weighted sum approaches, multiple objectives are combined into a single objective function using weights defined by the user which basically reflect the importance assigned to the objective [@halme1989]. The relationship between these objective weights and the achieved outcomes are indirect and problem-dependent, making it difficult to interpret how a given choice of weights translates into target attainment. ε-constraint methods designate one objective for optimization while converting others into constraints with prescribed bounds, which can lead to infeasibility when targets cannot be simultaneously satisfied [@revelle2004]. Interval-based multiobjective linear programming represents objectives or parameters as ranges rather than fixed values, shifting uncertainty handling into the optimization formulation but still requiring feasibility to be defined through hard bounds that may not align with aspirational targets [@halffmann2022]. Pareto-based approaches aim to enumerate or approximate the set of non-dominated solutions; a non-dominated solution is one for which no objective can be improved without worsening at least one other objective. This places the burden on the decision maker to explore and select a “best compromise” solution from a potentially large solution set [@halme1989; @revelle2004].

While these approaches are well established within multiobjective linear programming, they blur an important conceptual distinction that arises in many applied planning problems i.e. the difference between feasibility requirements (constraints) that must be satisfied and aspiration targets (goals) that may be violated at a known and acceptable cost. Scalarized objective formulations treat all quantities symmetrically as objectives to be optimized, whereas ε-constraint and interval-based methods essentially treat targets as constraints increasing the possibility of infeasible solutions. In contrast, goal linear programming introduces deviation variables that explicitly quantify under- and over-achievement relative to specified targets, allowing infeasible aspiration levels to be accommodated in a controlled and interpretable manner. This explicit representation of deviations aligns more naturally in the context of decision making, where targets serve as guiding benchmarks rather than strict constraints [@charnes1957; @orumie2014].
