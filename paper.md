...
PyGuLP is designed to formalize and standardize how weighted goal programming models are constructed and solved in applied research. The package provides a consistent structure for separating feasibility constraints from aspirational goals or targets, reflecting the distinction emphasized in the goal programming literature but often implemented manually in practice. Figure @fig:glp demostrates the framework for PyGuLP with PyGuLP separates feasibility constraints from aspirational goals.  For each goal, PyGuLP systematically introduces deviation variables and constructs the corresponding goal-linking constraint, ensuring that under- and over-achievement are represented explicitly. The resulting optimization problem remains a linear program and is solved using established solvers through the PuLP ecosystem. By returning achieved values and deviations in a structured form, PyGuLP supports interpretation, comparison across scenarios, and reuse across problem settings. 
![Figure 1: : Conceptual overview of Goal Linear Programming: Constraints(A) represent bounds of feasibility which are distinct from targets (B) or goals which may be violated for a cost. Goal programming introduces deviation variables to quantify under- and over-achievement and selects a feasible solution that minimizes weighted deviations across competing goals. Panel C shows the largest weight (W3) deviates least from the assigned target, whereas W1 and W2 show deviations relative to their magnitude.Solution still remains within feasibility limits as defined by hard constraints in panel A.](Graphical_explanation.png){#fig:glp width=90%}
